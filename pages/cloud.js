...
const slides = [{src:clouds,alt:"cloud computing"}, {src:aws,alt:"amazon web services"}, {src:google,alt:"google cloud"}, {src:azure,alt:"azure cloud"}, {src:oracle, alt:"oracle cloud"}, {src:ibm,alt:"ibm cloud"}];
export default function cloud({posts, faq}) {
    return(
    <>
    <Head>
        <title>Cloud Computing - CLUDBIT PRIVATE LIMITED</title>
        <meta name="description" content="OFFERING VARIOUS SERVICES IN Cloud Computing (using AWS Cloud, using GOOGLE cloud, using AZURE cloud, using Oracle cloud, using IBM cloud)" />
        <meta property="og:title" content="Cloud Computing - Cludbit Private Limited"/>
        <meta property="og:description" content="OFFERING VARIOUS SERVICES IN Cloud Computing (using AWS Cloud, using GOOGLE cloud, using AZURE cloud, using Oracle cloud, using IBM cloud)" />
        <meta property="og:url" content="https://www.cludbit.com/cloud"/>
        <meta property="og:type" content="website"/>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta name='keywords' content='AWS CLOUD, GOOGLE CLOUD, AZURE CLOUD, ORACLE CLOUD, IBM CLOUD, Android development, web development, ios development, hybrid application development, data analytics, data engineering, AI, ML, data management, security management, digital marketting'/>
    ...
    <Carousel autoSlide={true} autoSlideInterval={5000}>
            {slides.map((s)=>(
              <Image src={s.src} key={i} alt={s.alt}/>
            ))}
          </Carousel>
    </div>
    </div>
...
export async function getServerSideProps(ctx) {
  ...
  let response = await fetch(`${dev ? DEV_URL : PROD_URL}/api/cloud`);
  // extract the data
  let data = await response.json();

  // request posts from api
  let responses = await fetch(`${dev ? DEV_URL : PROD_URL}/api/cloud`);
  // extract the data
