...
const slides = [{src:ios, alt:"ios application development"}, {src:swift,alt:"swift programming language"}, {src:firebase, alt:'google firebase '},];
export default function iosapplicationdevelopment({posts, faq}) {
    return(
    <>
     <Head>
        <title>IOS Application Development - CLUDBIT PRIVATE LIMITED</title>
        <meta name="description" content="OFFERING VARIOUS SERVICES IN IOS Application DEVELOPMENT (using SWIFT programming, using google firebase)" />
        <meta property="og:title" content="IOS Application Development - Cludbit Private Limited"/>
        <meta property="og:description" content="OFFERING VARIOUS SERVICES IN IOS Application DEVELOPMENT (using SWIFT programming, using google firebase)" />
        <meta property="og:url" content="https://www.cludbit.com/ios-application-development"/>
        <meta property="og:type" content="website"/>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta name='keywords' content='swift, firebase, Android development, web development, ios development, hybrid application development, data analytics, data engineering, AI, ML, data management, security management, digital marketting'/>
...
export async function getServerSideProps(ctx) {
  ...
  let response = await fetch(`${dev ? DEV_URL : PROD_URL}/api/iosdevelopment`);
  // extract the data
  let data = await response.json();

  // request posts from api
  let responses = await fetch(`${dev ? DEV_URL : PROD_URL}/api/iosdevelopment`);
  // extract the data
